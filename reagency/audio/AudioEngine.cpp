...
// buffer-size-independent one-pole coefficient for a target time-constant tau (seconds)
...
static bool loadWav16(const std::string& path, std::vector<float>& out, float& srcSR) {
  ...
  char id[4]; uint32_t sz = 0; bool gotFmt = false, gotData = false;
  while (std::fread(id, 1, 4, f) == 4 && std::fread(&sz, 4, 1, f) == 1) {
    if (!std::memcmp(id, "fmt ", 4)) {
      ...
      std::fseek(f, 6, SEEK_CUR); std::fread(&b, 2, 1, f);
      ch = c; bits = b; rate = r; gotFmt = true;
      long skip = long(sz) - 16; if (skip > 0 && skip < (1 << 20)) std::fseek(f, skip, SEEK_CUR);
    } else if (!std::memcmp(id, "data", 4)) {
      if (!gotFmt || bits != 16 || ch < 1 || ch > 8 || rate == 0) { std::fclose(f); return false; }
      if (sz == 0 || sz > 200u * 1024u * 1024u) { std::fclose(f); return false; }
      long n = long(sz) / 2;
      std::vector<int16_t> raw(n);
      if (long(std::fread(raw.data(), 2, n, f)) != n) n = 0;
      int frames = (ch > 0) ? int(n / ch) : 0;
      out.resize(frames);
...
static bool parseNoteHz(const std::string& name, float& hz) {
  ...
  for (size_t i = 0; i + 1 < name.size(); ++i) {
    ...
    int oct = name[j] - '0', midi = (oct + 1) * 12 + semi[c - 'A'] + sharp;
    hz = 440.f * std::pow(2.f, (midi - 69) / 12.f); return true;
  }
  return false;
}
...
void AudioEngine::loadSamples(const std::string& assetDir) {
  ...
  for (const auto& b : bases) {
    ...
    while ((e = readdir(d)) != nullptr) {
      ...
      if (has("agogo") || has("gong") || has("bowl") || has("chime") || has("crotale") || has("tubular") || has("bell")) s.pitched = false;
      int idx = int(samples_.size()); samples_.push_back(std::move(s));
      if (has("organ") || has("pedal") || has("tuba") || has("bassoon") || has("contrabass") || has("timpani") || has("trombone")) add(rBass_, idx);
      else if (has("agogo") || has("gong") || has("bowl") || has("chime") || has("crotale") || has("tubular") || has("/bell") || has("_bell")) add(rBell_, idx);
      else if (has("psaltery") || has("bowed") || has("cello") || has("violin") || has("viola") || has("choir") || has("voice") || has("clarinet")) add(rTrace_, idx);
      else add(rPluck_, idx);   // kalimba/harp/glock/celesta/marimba/recorder/flute/...
    }
    closedir(d);
    if (!samples_.empty()) {
      std::fprintf(stderr, "[wosw audio] %zu CC0 samples from %s/  (bass=%d pluck=%d bell=%d trace=%d)\n",
                   samples_.size(), b.c_str(), rBass_.n, rPluck_.n, rBell_.n, rTrace_.n);
      return;
    }
  }
...
void AudioEngine::loadManifest(const std::string& assetDir) {
  ...
  for (const auto& b : bases) {
    ...
    std::string s; char buf[4096]; size_t n; while ((n = std::fread(buf, 1, sizeof(buf), f)) > 0) s.append(buf, n);
    std::fclose(f);
    auto pos = s.find("\"root_hz\"");
    if (pos != std::string::npos) { double v = std::atof(s.c_str() + s.find(':', pos) + 1); if (v > 20.0 && v < 4000.0) root_ = float(v); }
    pos = s.find("\"mode\"");
...
float AudioEngine::degHz(int degree) const {
  int idx = degree % modeN_, oct = degree / modeN_; if (idx < 0) { idx += modeN_; oct -= 1; }
  return root_ * std::pow(2.f, (modeBuf_[idx] + 12 * oct) / 12.f);
}
int AudioEngine::nodeDegree(int node, int cluster) const {
  unsigned h = unsigned(node) * 2654435761u; int d = int(h % unsigned(modeN_ * 3));
  if (cluster > 0 && (cluster % 2 == 1)) d += modeN_; return d;
}
...
static float slotRate(float depth, float hes, float act) {
  float r = (1.6f + 1.3f * depth) * (1.f - 0.28f * hes) * (1.f + 0.20f * act);   // calmer than v2
  return r < 0.5f ? 0.5f : (r > 4.5f ? 4.5f : r);
}
...
void AudioEngine::onArrival(int node, int type, float density, int cluster) {
  ...
  e.hz = degHz(nodeDegree(node, cluster)); e.amp = 0.10f + 0.10f * simDensity_;   // quieter ping (bed)
  e.pan = cFocusPan_.load(std::memory_order_relaxed); push(e);
...
void AudioEngine::igniteArp(const std::vector<std::pair<int, float>>& nbrs, int cluster) {
  ...
  for (int step = 0; step < L; ++step) {
    float w[13], sum = 0.f;
    for (int i = 0; i < 13; ++i) {
      int dd = dStep[i]; float ww = std::exp(-std::fabs(float(dd)) / 3.f);
      ww += hes * 0.6f * (std::abs(dd) >= 4 ? 1.f : 0.f);
      if ((melDeg_ + dd) % modeN_ == gravDeg % modeN_) ww += 0.8f;
      if (lastLeap_ && std::abs(dd) > 2) ww = 0.f;
      w[i] = ww; sum += ww;
    }
    float r = frnd() * sum, acc = 0.f; int pick = 6;
    for (int i = 0; i < 13; ++i) { acc += w[i]; if (r <= acc) { pick = i; break; } }
    int dd = dStep[pick]; lastLeap_ = std::abs(dd) > 2; melDeg_ += dd;
    melDeg_ = std::max(-modeN_, std::min(modeN_ * 3, melDeg_));
    float edgeW = nbrs.empty() ? 0.5f : nbrs[step % nbrs.size()].second;
    bool accent = (step == 0) || (melDeg_ % modeN_ == gravDeg % modeN_);    // musical accents
    int layer = (edgeW > 0.80f) ? 1 : 0;
    // encode amp in hz? no — store layer; amp computed at emit from accent flag via sign trick:
    mel_.push_back({degHz(melDeg_ + octShift), accent ? (layer + 10) : layer});  // +10 marks accent
  }
}

void AudioEngine::traceOn(int slot, int node, float pan) {
  if (!ready()) return;
  Ev e{}; e.kind = EV_TRACE_ON; e.layer = 3; e.hz = degHz(nodeDegree(node, 0)) * 0.5f; e.amp = 0.12f; e.pan = pan; e.islot = slot; push(e);
}
...
void AudioEngine::whisper(const std::string& word, int node, float pan) {
  ...
  float f0, f1, f2;
  if (word.empty() || syl == 0) { static const char vs[5] = {'a','e','i','o','u'}; formantFor(vs[(node>=0?node:0)%5], f0, f1, f2); syl = (node>=0?node:0)%3+1; }
  else formantFor(char(firstV), f0, f1, f2);
  syl = syl < 1 ? 1 : (syl > 5 ? 5 : syl);
  Ev e{}; e.kind = EV_WHISPER; e.layer = 4; e.amp = 0.55f; e.pan = pan; e.islot = syl; e.a = f0; e.b = f1; e.c = f2; push(e);  // LEAD level
}
...
void AudioEngine::update(float dt, float hesitation, float depth, float progress, float focusPan) {
  ...
  activity_ *= std::pow(0.5f, dt / 0.8f); cActivity_.store(activity_, std::memory_order_relaxed);
  (void)progress;

  // STOCHASTIC mood scheduler — mood changes happen at FIBONACCI-second intervals (golden macro-form)
  ...
  moodTimer_ -= dt;
  if (moodTimer_ <= 0.f) { moodTimer_ = FIB[int(sr() * 6.f) % 6]; mbTgt_ = sr(); mdTgt_ = sr(); mwTgt_ = sr(); }
  float mc = std::min(1.f, dt * 0.25f);
  moodBright_ += (mbTgt_ - moodBright_) * mc; moodDens_ += (mdTgt_ - moodDens_) * mc; moodWet_ += (mwTgt_ - moodWet_) * mc;
  ...
  if (melIdx_ < mel_.size()) {
    float period = 1.f / (1.6f * slotRate(depth, hesitation, activity_));   // a touch slower, breathes
    melTimer_ += dt;
    while (melIdx_ < mel_.size() && melTimer_ >= period) {
      melTimer_ -= period;
      int lay = mel_[melIdx_].second; bool accent = lay >= 10; lay %= 10;
      Ev e{}; e.kind = EV_NOTE; e.layer = lay; e.hz = mel_[melIdx_].first;
      e.amp = (accent ? 0.11f : 0.06f);                                     // quiet bed; accents lift
      e.pan = melPan_ + 0.18f * (float(melIdx_ % 5) - 2.f); push(e); ++melIdx_;
    }
  }
}
...
int AudioEngine::allocCapped(int layer, int cap) {
  int cnt = 0, oldest = -1; float oldAge = -1.f;
  for (int i = 0; i < NVOX; ++i) if (vox_[i].on && vox_[i].layer == layer) { ++cnt; if (vox_[i].age > oldAge) { oldAge = vox_[i].age; oldest = i; } }
  if (cnt < cap) { int f = allocVoice(); if (f >= 0) return f; }
  return oldest >= 0 ? oldest : allocVoice();
}
...
void AudioEngine::trigger(const Ev& e) {
  ...
  int i;
  if (e.layer == 0 || e.layer == 1) i = allocCapped(e.layer, 4); else i = allocVoice();
  if (i < 0) return;
  ...
  auto setSample = [&](int sidx) {
    const Sample& s = samples_[sidx]; v.smp = s.data.data(); v.smpLen = int(s.data.size()); v.smpPos = 0.f;
    float ssr = s.srcSR > 0.f ? s.srcSR : float(sr_), ratio = s.pitched ? (v.hz / s.rootHz) : 1.f;
    while (ratio > 2.f) ratio *= 0.5f; while (ratio < 0.5f) ratio *= 2.f;     // fold into one octave
    v.smpRate = ratio * (ssr / float(sr_));
  };
  ...
  switch (e.layer) {
    ...
    case 5: { int s = pickSample(rBass_, v.hz); if (s >= 0) { setSample(s); v.life = std::min(5.0f, float(v.smpLen)/float(sr_)); } else { v.life = 4.0f; setPartials(3, 0.0003f); } v.atk = 0.25f; break; }  // organ pedal
    case 6: { v.life = 0.26f; v.atk = 0.001f; break; }   // KICK (synth)
    case 4: {
      v.syl = e.islot < 1 ? 1 : e.islot; v.life = 1.5f + 0.55f * v.syl; v.body = 0.62f; v.atk = 0.05f;
      v.fmt[0] = e.a; v.fmt[1] = e.b; v.fmt[2] = e.c; const float bw[3] = {90.f, 110.f, 140.f};
      for (int k = 0; k < 3; ++k) { float r = std::exp(-kPI*bw[k]/float(sr_)); v.fcoef[k][0]=(1.f-r); v.fcoef[k][1]=-2.f*r*std::cos(kTAU*v.fmt[k]/float(sr_)); v.fcoef[k][2]=r*r; }
      break;
    }
...
float AudioEngine::tickVoice(Voice& v, float isr, float depth, float bright) {
  ...
  if (v.layer == 6) {                                    // KICK
    float fk = 48.f + 100.f * std::exp(-v.age / 0.030f);
    v.phase += fk * isr; if (v.phase >= 1.f) v.phase -= 1.f;
    float click = (v.age < 0.004f) ? (1.f - v.age / 0.004f) * (vrand(v.grng) * 2.f - 1.f) * 0.6f : 0.f;
    float env = std::exp(-v.age / 0.10f);
    return (std::sin(kTAU * v.phase) * env + click) * v.amp;
  }

  if (v.smp) {                                           // sampled
    if (v.smpPos >= v.smpLen - 2) { v.on = false; return 0.f; }
    int i0 = int(v.smpPos); float fr = v.smpPos - i0; float s = v.smp[i0]*(1.f-fr) + v.smp[i0+1]*fr; v.smpPos += v.smpRate;
    float env;
    if (v.layer == 3 || v.layer == 5) env = (u < (v.atk / v.life)) ? (u / std::max(1e-4f, v.atk / v.life)) : (u < 0.6f ? 1.f : std::max(0.f, 1.f - (u - 0.6f) / 0.4f));
    else { float a = v.atk / v.life; env = (u < a) ? (u / std::max(1e-4f, a)) : (u > 0.85f ? (1.f - (u - 0.85f) / 0.15f) : 1.f); }
    return s * env * v.amp;
  }

  if (v.layer == 4) {                                    // whisper
    float env;
    ...
    for (int k = 0; k < 3; ++k) { float y = v.fcoef[k][0]*nz - v.fcoef[k][1]*v.fz_[k][0] - v.fcoef[k][2]*v.fz_[k][1]; v.fz_[k][1]=dn(v.fz_[k][0]); v.fz_[k][0]=dn(y); s += w[k]*y; }
    return s * env * v.amp * 1.5f;
  }

  v.phase += v.hz * isr; if (v.phase >= 1.f) v.phase -= std::floor(v.phase);
  float s = 0.f; const float nyq = 0.45f * float(sr_);
  for (int k = 0; k < v.K; ++k) { float pf = v.hz * v.pm[k]; if (pf >= nyq) break; s += v.pa[k] * std::sin(kTAU * v.phase * v.pm[k]); }
  s /= v.pnorm;
  float env;
...
void AudioEngine::fireKick() {
  int i = allocCapped(6, 2); if (i < 0) return;
  Voice& v = vox_[i]; v = Voice{}; v.on = true; v.layer = 6; v.life = 0.26f; v.age = 0.f;
  v.amp = (euStep_ == 0) ? 0.62f : 0.46f;                // accent the downbeat
  v.grng = rng_ ^ (uint32_t(i) * 2654435761u);
}
...
void AudioEngine::render(al::AudioIOData& io) {
  ...
  // cluster change -> a low organ pedal note (harmonic glue, not per-pulse)
  int cl = cClusterId_.load(std::memory_order_relaxed);
  if (cl != lastClusterId_) { lastClusterId_ = cl; Ev p{}; p.kind = EV_NOTE; p.layer = 5; p.hz = jiBassHz(); p.amp = 0.06f; p.pan = 0.f; trigger(p); }
  ...
  // buffer-size-independent glides (mood/tide)
  ...
  subAmp_   += ((0.045f + 0.08f * (1.f - depth)) - subAmp_) * gco(7.f, nf, sr_);
  shepGain_ += ((0.016f + 0.012f * depth + 0.012f * tension_) - shepGain_) * gco(1.f, nf, sr_);   // noise-bed quiet
  shepRateTgt_ = (hes > 0.55f ? -1.f : 1.f) * (1.f / (16.f - 8.f * hes));
  shepRate_ += (shepRateTgt_ - shepRate_) * gco(8.f, nf, sr_);

  // duck the bed under the whisper lead
  bool whisperOn = false; for (auto& v : vox_) if (v.on && v.layer == 4) { whisperOn = true; break; }
  duckGain_ += ((whisperOn ? 0.45f : 1.f) - duckGain_) * gco(0.25f, nf, sr_);
  ...
  const float isr = 1.f / float(sr_), master = 0.44f;
  const float slot = slotRate(depth, hes, act);
  const float fifthA = 1.5f, fifthB = 1.5f - 0.006f * tension_;   // two fifths -> gentle ~1-2Hz beating
  const float padRatio[PADN] = {1.f, kJI[2], fifthA, 2.f};

  for (int f = 0; f < nf; ++f) {
    float bedL = 0.f, bedR = 0.f, leadL = 0.f, leadR = 0.f, lowMono = 0.f, revSend = 0.f;
    panLFO_ += 0.05f * isr; if (panLFO_ >= 1.f) panLFO_ -= 1.f;
    float drift = 0.25f * std::sin(kTAU * panLFO_);

    euPhase_ += slot * isr;
    if (euPhase_ >= 1.f) { euPhase_ -= 1.f; euStep_ = (euStep_ + 1) % euLen_; if ((euMask_ >> euStep_) & 1u) fireKick(); }

    // sub-bass pedal (centred, dry)
    ...
    lowMono += subLp_ * subAmp_;

    // JI pad bed
    for (int p = 0; p < PADN; ++p) {
      float tgt = padRoot * padRatio[p]; padHz_[p] += (tgt - padHz_[p]) * 0.0006f;
      ...
      padLp_[p] = dn(padLp_[p] + (0.12f + 0.40f * depth) * (s - padLp_[p]));
      float tgtAmp = (p < 2) ? 0.10f : 0.10f * padBloom_; padAmp_[p] += (tgtAmp - padAmp_[p]) * 0.001f;
      float a = padAmp_[p] * padLp_[p];
      float pan = (p == 0) ? 0.f : (p == 1 ? -0.55f : (p == 2 ? 0.55f : 0.78f)); float pp = 0.5f * (pan + 1.f);
      bedL += a * std::cos(pp * 1.5707963f); bedR += a * std::sin(pp * 1.5707963f); revSend += a * 0.8f;
    }
    // beating fifth (a second, slightly detuned P5) for shimmer instead of a mistuned single partial
    ...
    // Shepard ladder + airy noise bed (the background wash)
    ...
    float bedair = sh + nzLp2_ * 0.020f;
    bedL += bedair * (0.7f + drift); bedR += bedair * (0.7f - drift); revSend += bedair * 1.2f;

    // grain cloud (Poisson)
    grainTimer_ -= isr;
    if (grainTimer_ <= 0.f) {
      float lam = 5.f + 30.f * clamp01(0.3f * act + 0.3f * (1.f - depth) + 0.4f * hes + 0.4f * md);
      grainTimer_ += -std::log(std::max(1e-6f, frand())) / lam;
      int gi = allocVoice();
      if (gi >= 0) { Voice& g = vox_[gi]; g = Voice{}; g.on = true; g.layer = 2; g.hz = degHz(nodeDegree(int(rng_ & 1023), -1)) * (depth > 0.55f ? 2.f : 1.f);
        g.K = 2; g.pm[0] = 1; g.pm[1] = 2; g.pa[0] = 1; g.pa[1] = 0.4f; g.pnorm = 1.4f; g.amp = 0.03f + 0.03f * act; g.life = 0.02f + 0.10f * frand();
        g.pan = (0.4f + 0.6f * clamp01(hes)) * (2.f * frand() - 1.f); g.grng = rng_ ^ (uint32_t(gi) * 40503u); }
    }

    // pooled voices -> lead (whisper) / low (kick) / bed (everything else)
    for (auto& v : vox_) {
      if (!v.on) continue;
      float s = tickVoice(v, isr, depth, moodLP_);
      if (v.layer == 4) {                                // WHISPER lead (wide, prominent)
        float pp = 0.5f * (v.pan + drift + 1.f); pp = pp < 0 ? 0 : (pp > 1 ? 1 : pp);
        leadL += s * std::cos(pp * 1.5707963f); leadR += s * std::sin(pp * 1.5707963f); revSend += s * 1.4f;
      } else if (v.layer == 6) { lowMono += s; }          // kick (dry, centre)
      else {
        float pan = v.pan + (v.layer == 2 ? drift : 0.f); float pp = 0.5f * (pan + 1.f); pp = pp < 0 ? 0 : (pp > 1 ? 1 : pp);
        bedL += s * std::cos(pp * 1.5707963f); bedR += s * std::sin(pp * 1.5707963f); revSend += s * 0.5f;
      }
    }

    float w1 = 0.f, w2 = 0.f; reverb_(revSend * 0.5f, w1, w2, 0.6f);     // sub+kick NOT sent (low stays clean)
    float L = duckGain_ * bedL + leadL + lowMono + reverbWet_ * w1;
    float R = duckGain_ * bedR + leadR + lowMono + reverbWet_ * w2;
    io.out(0, f) = std::tanh(L * master);
    if (nch > 1) io.out(1, f) = std::tanh(R * master);
