...
void AudioEngine::render(al::AudioIOData& io) {
  ...
  // VOICE DUCKING REMOVED: the bed + low end no longer dip under layer-4 voices (the duck read as the
  // music "dropping out" when THEM speak). The voice now sits on top at its own gain (leadL/leadR).
  // whisperOn still drives the glitch-density gap logic; the kick->sub sidechain (kickDuck_) is separate.
